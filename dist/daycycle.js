...
export function createDayCycle(scene,sun,hemi,water,player){
 ...
 return {get state(){return {hour,speed}},restore(saved){hour=saved.hour;speed=saved.speed;slider.value=hour;document.getElementById('timeSpeed').value=String(speed);},update(dt){hour=(hour+dt*24/360*speed)%24;const {elevation,day,dusk}=lightingAt(hour),a=(hour-6)/24*Math.PI*2;
