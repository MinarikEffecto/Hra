...
export function createBuriedCacheUI(game, {toast, sound, hud}) {
  ...
  panel.setAttribute('role', 'dialog'); panel.setAttribute('aria-modal', 'true'); panel.setAttribute('aria-labelledby', 'bunkerTitle'); panel.setAttribute('aria-describedby', 'bunkerStory bunkerNeeds');
  ...
  open.onclick = () => {
    ...
    if (!game.grounded) { toast('Nejprve se postav na zem.'); return; }
  ...
  $('bunkerRecover').onclick = () => { if (cache.recover()) { hud(); sound('pickup'); toast('K–03: získal jsi 3 rudy a 2 porce jídla.'); refresh(); $('bunkerClose').focus(); } };
  addEventListener('keydown', e => {
    if (panel.hidden) return;
    if (e.key === 'Escape') { e.preventDefault(); close(); }
    if (e.key === 'Tab') {
      const buttons = [...panel.querySelectorAll('button:not([hidden]):not(:disabled)')];
